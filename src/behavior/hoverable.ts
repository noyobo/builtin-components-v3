...
export const hoverable = <T extends Constructor<BaseElement>>(superClass: T): Constructor<HoverableInterface> & T => {
  class HoverableElement extends superClass {
    ...
    @property({ type: String, attribute: 'hover-class' }) hoverClass = DEFAULT_HOVER_CLASS;

    /** 指定是否阻止本节点的祖先节点出现点击态 */
    @property({ type: Boolean, attribute: 'hover-stop-propagation' }) hoverStopPropagation = false;

    /** 按住后多久出现点击态，单位毫秒 */
    @property({ type: Number, attribute: 'hover-start-time' }) hoverStartTime = 20;

    /** 手指松开后点击态保留时间，单位毫秒 */
    @property({ type: Number, attribute: 'hover-stay-time' }) hoverStayTime = 70;
